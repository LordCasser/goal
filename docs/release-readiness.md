...
v0.1.8 包含回收站、开放式长期周期、事项详情中的直接关联子项，以及统一的日期/时间选择器。发布流水线负责前端测试、版本合同校验、六目标构建和产物校验；自动化及集成覆盖不替代人工桌面验收。

回收站变更的手工验收仍有两项未完成：确认永久删除的完整界面路径，以及大计划快照的数据库大小和删除/恢复耗时检查。发布说明保留这一限制，不将其描述为已验收。macOS 包继续使用固定自签名身份且未公证，Windows 安装包未签名；六目标构建也不代表每个平台都完成原生 GUI 验收。
