...
主工作台使用普通鼠标滚轮时默认横向浏览。鼠标主键点击列表内容区后，该列表改为上下滚动，标题旁只显示中性的“↕”图标；悬停可查看当前语言的滚动说明，辅助技术可读取相同标签。时间导航只显示箭头，悬停可查看说明。点击标题或列表外恢复横向浏览。空列表、滚到顶部或底部都不会自动换轴，Tab、自动聚焦和悬停也不会切换模式。明显横向的触控板手势、输入控件和缩放操作保持原有行为。
