...
系统 SHALL 支持在周期之间移动任务、在同级内重排，并在移动前校验目标容器的可变性。排序的兄弟组 SHALL 与编辑器展示树一致：未关联和关联外部目标的可见根任务属于同一组；同周期子步骤按自己的父任务分组。排序 SHALL 保留已有归属。Later 事项恢复到计划时 SHALL 按持久化顺序排在空输入行之前，并接续现有事项；该恢复位置 MUST 在事务提交后仍保持。计划编辑器中的空输入行 SHALL 显示在所有可见事项之后，其他插入路径继续保留各自既有的持久排序语义。

#### Scenario: 不同归属的周任务排序
- **WHEN** 两个周任务分别关联不同长期目标，用户通过行菜单调整它们在同一周列表中的顺序
