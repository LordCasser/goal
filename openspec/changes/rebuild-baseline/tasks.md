...
对应规范：`openspec/specs/telemetry/spec.md`

- [x] 9.1 设置项：遥测开关（默认按规范要求处理：缺失即视为未启用）
- [x] 9.2 `telemetry/mod.rs`：事件定义（只允许枚举值、计数、时长），上报通道，超时与丢弃策略
- [ ] 9.3 首次运行告知 + 关闭入口；关闭后完全静默
- [x] 9.4 上报不在业务关键路径同步等待；失败静默丢弃，不影响任何功能
- [x] 9.5 匿名标识独立于设备指纹与授权标识生成
- [x] 9.6 诊断信息导出：版本、迁移版本、错误类别，**不含计划内容**
- [x] 9.7 测试：关闭后零出网；事件载荷不包含任何自由文本（用包含敏感串的标题做断言）；端点不可达时静默；批量超时不影响业务
...
- [ ] 11.6 手工验证遥测：关闭后确认零出网；打开后确认事件里没有计划文本
