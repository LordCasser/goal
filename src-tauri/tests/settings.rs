...
#[test]
fn relation_lines_default_to_off_and_persist_across_reopen() {
    let f = common::TestDb::open();
    assert!(!settings::get(&f.db).unwrap().show_relation_lines);

    settings::set_show_relation_lines(&f.db, true).unwrap();
    assert!(settings::get(&f.db).unwrap().show_relation_lines);
    ...
    settings::set_show_relation_lines(&reopened, false).unwrap();
    assert!(!settings::get(&reopened).unwrap().show_relation_lines);
}
