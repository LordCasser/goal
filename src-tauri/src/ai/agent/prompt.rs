...
pub fn build_system_instruction(skill: AgentSkill, cycle_type: CycleType) -> AppResult<String> {
    ...
    Ok(format!(
        ...
        COMMON,
        ROUTING,
        skills::load(scenario)?,
        crate::ai::persona::load()?
    ))
}
...
- Preserve user-authored goal titles and content. The application supplies a default reply language; an explicit user language request takes precedence.
...
mod tests {
    ...
    fn week_and_day_load_distinct_workflows_with_shared_contract() {
        let week =
            build_system_instruction(AgentSkill::ShortTermPlanning, CycleType::Week).unwrap();
