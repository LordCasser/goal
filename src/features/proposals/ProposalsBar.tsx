...
export function ProposalsBar({onOpen}:{onOpen:(cycleId:string)=>void}) {
  ...
  return <Button aria-label={`${total} 项待确认`} variant="ghost" size="compact" className="window-pending h-7 gap-1.5 px-2 text-caption" onClick={()=>onOpen(counts[0]!.id)} title={`${total} 项改动：在 Coach 中查看并确认`}><span className="rounded bg-focus-surface px-1.5 text-focus">{total}</span><span className="window-tool-label">项待确认</span></Button>;
}
