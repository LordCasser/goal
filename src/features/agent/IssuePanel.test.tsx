...
describe("plan diagnostics", () => {
  it("distinguishes an unchecked plan from a successful empty AI report", async () => {
    mockBackend(); mount();
    expect(await screen.findByText("日计划 · 2026-09-15")).toBeTruthy();
  ...
  it("shows real pending feedback then the model, checked count and completed result", async () => {
    ...
    mount(); await screen.findByText("日计划 · 2026-09-15");
  ...
  it("never turns a provider failure into all-clear and retains earlier findings", async () => {
    ...
    mount(); await screen.findByText(finding.title);
    fireEvent.click(screen.getByRole("button", { name: "AI 检查" }));
    expect(await screen.findByText("AI 检查未完成")).toBeTruthy();
    expect(screen.getByText("Network unavailable")).toBeTruthy(); expect(screen.getByText(finding.title)).toBeTruthy();
    expect(screen.queryByText("本次检查未发现需要处理的问题。")).toBeNull();
  });
  it("makes stale findings and pending Coach previews explicit", async () => {
    mockBackend(report({ ai_status: "stale", pending_count: 1 })); mount();
    expect(await screen.findByText("计划或模型已变化，请重新运行 AI 检查。")).toBeTruthy();
    expect(screen.getByText(/1 项 Coach 预览尚未确认/)).toBeTruthy();
  });
  it("locates the exact task and carries the diagnostic into a Coach draft without sending", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "与 Coach 讨论" }));
    expect(discuss).toHaveBeenCalledWith("c1", expect.stringContaining("任务「Prototype」"), "t1");
  ...
  it.each([
    ...
    const row = (await screen.findByText(finding.title)).closest("li")!;
