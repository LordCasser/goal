...
export function ParentGoalPicker({ task, relations, locked, nested }: {
  ...
  const parentLabel = relations.cycles.get(task.cycle_id)?.type === "week" ? t("parent.labelLongTerm") : t("parent.labelWeekly");
  ...
  return <>
    <button ref={anchorRef} type="button" className="task-color-control"
      ...
      title={parent ? t("parent.linked", { title: parent.title }) : t("parent.linkA", { parent: parentLabel })}
    ...
    <Popover open={open} onClose={() => setOpen(false)} anchorRef={anchorRef} label={t("parent.menu", { parent: parentLabel })} className="w-[300px]">
      ...
      {nested ? <p className="px-3 py-3 text-menu text-secondary">{t("parent.belongs", { parent: parent?.title ?? t("parent.current") })}</p> : <>
        ...
        {parent && <PopoverItem disabled={locked || saving} className="border-t border-light" onSelect={() => void choose(null)}>{t("parent.unlink", { parent: parentLabel })}</PopoverItem>}
      </>}
