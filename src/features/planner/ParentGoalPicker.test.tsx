...
function mount(task = week, options = { locked: false, nested: false }) {
  const relations: RelationView = { tasks: new Map([[root.id, root], [task.id, task]]), cycles: new Map([["m", { id: "m", type: "month" } as Cycle], ["w", { id: "w", type: "week", parent_id: "m" } as Cycle]]), selectedId: null, highlighted: new Set(), select: vi.fn(), preview: vi.fn(), setDragging: vi.fn() };
