...
export function ReminderSettingsSection(): JSX.Element {
  ...
  return (
    <section aria-label="提醒设置" className="flex flex-col gap-6">
      <div className="flex flex-col gap-3">
        <Checkbox checked={quietEnabled} onChange={setQuietEnabled}>
          <span className="text-body font-medium text-primary">免打扰时段</span>
        </Checkbox>
        {quietEnabled && (
      ...
      <div className="flex flex-col gap-3">
        <Checkbox checked={dailyEnabled} onChange={setDailyEnabled}>
          <span className="text-body font-medium text-primary">每日计划提醒</span>
        </Checkbox>
        {dailyEnabled && (
          <Input
            type="time"
